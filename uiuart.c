...
//*****************************************************************************
//
// Output ring buffer.  Buffer is full if ui_gui32UARTTxReadIndex is one ahead of
// ui_gui32UARTTxWriteIndex.  Buffer is empty if the two indices are the same.
//
//*****************************************************************************
static unsigned char ui_gpcUARTTxBuffer[UART_TX_BUFFER_SIZE];
static volatile uint32_t ui_gui32UARTTxWriteIndex = 0;
static volatile uint32_t ui_gui32UARTTxReadIndex = 0;

//*****************************************************************************
//
// Input ring buffer.  Buffer is full if ui_gui32UARTTxReadIndex is one ahead of
// ui_gui32UARTTxWriteIndex.  Buffer is empty if the two indices are the same.
//
//*****************************************************************************
static unsigned char ui_gpcUARTRxBuffer[UART_RX_BUFFER_SIZE];
static volatile uint32_t ui_gui32UARTRxWriteIndex = 0;
static volatile uint32_t ui_gui32UARTRxReadIndex = 0;
...
//
//*****************************************************************************
#define uiTX_BUFFER_USED          (uiGetBufferCount(&ui_gui32UARTTxReadIndex,  \
                                                &ui_gui32UARTTxWriteIndex, \
                                                UART_TX_BUFFER_SIZE))
#define uiTX_BUFFER_FREE          (UART_TX_BUFFER_SIZE - uiTX_BUFFER_USED)
#define uiTX_BUFFER_EMPTY         (uiIsBufferEmpty(&ui_gui32UARTTxReadIndex,   \
                                               &ui_gui32UARTTxWriteIndex))
#define uiTX_BUFFER_FULL          (uiIsBufferFull(&ui_gui32UARTTxReadIndex,  \
                                              &ui_gui32UARTTxWriteIndex, \
                                              UART_TX_BUFFER_SIZE))
#define uiADVANCE_TX_BUFFER_INDEX(Index) \
...
//
//*****************************************************************************
#define uiRX_BUFFER_USED          (uiGetBufferCount(&ui_gui32UARTRxReadIndex,  \
                                                &ui_gui32UARTRxWriteIndex, \
                                                UART_RX_BUFFER_SIZE))
#define uiRX_BUFFER_FREE          (UART_RX_BUFFER_SIZE - uiRX_BUFFER_USED)
#define uiRX_BUFFER_EMPTY         (uiIsBufferEmpty(&ui_gui32UARTRxReadIndex,   \
                                               &ui_gui32UARTRxWriteIndex))
#define uiRX_BUFFER_FULL          (uiIsBufferFull(&ui_gui32UARTRxReadIndex,  \
                                              &ui_gui32UARTRxWriteIndex, \
                                              UART_RX_BUFFER_SIZE))
#define uiADVANCE_RX_BUFFER_INDEX(Index) \
...
//
//*****************************************************************************
static const char * const ui_gpcHex = "0123456789abcdef";
...
//
//*****************************************************************************
static uint32_t ui_gui32PortNum;
#endif
...
#ifdef UART_BUFFERED
static bool
uiIsBufferFull(volatile uint32_t *pui32Read,
...
#ifdef UART_BUFFERED
static bool
uiIsBufferEmpty(volatile uint32_t *pui32Read,
...
#ifdef UART_BUFFERED
static uint32_t
uiGetBufferCount(volatile uint32_t *pui32Read,
               volatile uint32_t *pui32Write, uint32_t ui32Size)
{
    uint32_t ui32Write;
    uint32_t ui32Read;
...
#ifdef UART_BUFFERED
static void
uiUARTPrimeTransmit(uint32_t ui32Base)
{
    //
    // Do we have any data to transmit?
    //
    if(!uiTX_BUFFER_EMPTY)
    {
        //
        // Disable the UART interrupt.  If we don't do this there is a race
        // condition which can cause the read index to be corrupted.
        //
        MAP_IntDisable(ui_gui32UARTInt[ui_gui32PortNum]);

        //
        // Yes - take some characters out of the transmit buffer and feed
        // them to the UART transmit FIFO.
        //
        while(MAP_UARTSpaceAvail(ui32Base) && !uiTX_BUFFER_EMPTY)
        {
            MAP_UARTCharPutNonBlocking(ui32Base,
                                      ui_gpcUARTTxBuffer[ui_gui32UARTTxReadIndex]);
            uiADVANCE_TX_BUFFER_INDEX(ui_gui32UARTTxReadIndex);
        }

        //
        // Reenable the UART interrupt.
        //
        MAP_IntEnable(ui_gui32UARTInt[ui_gui32PortNum]);
    }
}
#endif
...
//*****************************************************************************
void
uiUARTxConfig(uint32_t ui32PortNum, uint32_t ui32Baud, uint32_t ui32SrcClock)
{
    //
    // Check the arguments.
    //
    ...
    //
    if(!MAP_SysCtlPeripheralPresent(ui_gui32UARTPeriph[ui32PortNum]))
    {
        return;
    }
    ...
    //
    ui_gui32Base = ui_gui32UARTBase[ui32PortNum];

    //
    // Enable the UART peripheral for use.
    //
    MAP_SysCtlPeripheralEnable(ui_gui32UARTPeriph[ui32PortNum]);

    //
    // Configure the UART for 115200, n, 8, 1
    //
    MAP_UARTConfigSetExpClk(ui_gui32Base, ui32SrcClock, ui32Baud,
    ...
    //
    MAP_UARTFIFOLevelSet(ui_gui32Base, UART_FIFO_TX1_8, UART_FIFO_RX1_8);
    ...
    //
    MAP_UARTIntDisable(ui_gui32Base, 0xFFFFFFFF);
    MAP_UARTIntEnable(ui_gui32Base, UART_INT_RX | UART_INT_RT);
    MAP_IntEnable(ui_gui32UARTInt[ui32PortNum]);
#endif

    //
    // Enable the UART operation.
    //
    MAP_UARTEnable(ui_gui32Base);
}
...
//*****************************************************************************
int
uiUARTwrite(const char *pcBuf, uint32_t ui32Len)
{
#ifdef UART_BUFFERED
    unsigned int uIdx;
    ...
    for(uIdx = 0; uIdx < ui32Len; uIdx++)
    {
        ...
        if(pcBuf[uIdx] == '\n')
        {
            if(!uiTX_BUFFER_FULL)
            {
                ui_gpcUARTTxBuffer[ui_gui32UARTTxWriteIndex] = '\r';
                uiADVANCE_TX_BUFFER_INDEX(ui_gui32UARTTxWriteIndex);
            }
            else
            {
                //
        ...
        if(!uiTX_BUFFER_FULL)
        {
            ui_gpcUARTTxBuffer[ui_gui32UARTTxWriteIndex] = pcBuf[uIdx];
            uiADVANCE_TX_BUFFER_INDEX(ui_gui32UARTTxWriteIndex);
        }
        else
        {
    ...
    if(!uiTX_BUFFER_EMPTY)
    {
        uiUARTPrimeTransmit(ui_gui32Base);
        MAP_UARTIntEnable(ui_gui32Base, UART_INT_TX);
    }
    ...
    for(uIdx = 0; uIdx < ui32Len; uIdx++)
    {
        ...
        if(pcBuf[uIdx] == '\n')
        {
            MAP_UARTCharPut(ui_gui32Base, '\r');
        }

        //
        // Send the character to the UART output.
        //
        MAP_UARTCharPut(ui_gui32Base, pcBuf[uIdx]);
    }
...
//*****************************************************************************
int
uiUARTgets(char *pcBuf, uint32_t ui32Len)
{
#ifdef UART_BUFFERED
    ...
    while(1)
    {
        ...
        {
            cChar = ui_gpcUARTRxBuffer[ui_gui32UARTRxReadIndex];
            uiADVANCE_RX_BUFFER_INDEX(ui_gui32UARTRxReadIndex);
    ...
    while(1)
    {
        ...
        //
        cChar = MAP_UARTCharGet(ui_gui32Base);
        ...
        if(ui32Count < ui32Len)
        {
            ...
            //
            MAP_UARTCharPut(ui_gui32Base, cChar);
        }
    }
...
uiUARTgetc(void)
{
    ...
    // the caller.
    //
    return(MAP_UARTCharGet(ui_gui32Base));
#endif
}
...
//*****************************************************************************
void
uiUARTvprintf(const char *pcString, va_list vaArgP)
{
...
convert:
                    ...
                    {
                        pcBuf[ui32Pos++] =
                            ui_gpcHex[(ui32Value / ui32Idx) % ui32Base];
                    }

                    //
                    // Write the string.
                    //
                    uiUARTwrite(pcBuf, ui32Pos);
...
//*****************************************************************************
void
uiUARTprintf(const char *pcString, ...)
{
    va_list vaArgP;
...
int
uiUARTRxBytesAvail(void)
{
    return(uiRX_BUFFER_USED);
}
#endif
...
//*****************************************************************************
int
uiUARTTxBytesFree(void)
{
    return(uiTX_BUFFER_FREE);
}
#endif
...
#if defined(UART_BUFFERED) || defined(DOXYGEN)
int
uiUARTPeek(unsigned char ucChar)
{
    int iCount;
    int iAvail;
    ...
    //
    iAvail = (int)uiRX_BUFFER_USED;
    ui32ReadIndex = ui_gui32UARTRxReadIndex;
    ...
    {
        if(ui_gpcUARTRxBuffer[ui32ReadIndex] == ucChar)
        {
            //
        ...
        else
        {
            ...
            //
            uiADVANCE_RX_BUFFER_INDEX(ui32ReadIndex);
        }
    }
...
uiUARTFlushRx(void)
{
    ...
    //
    ui_gui32UARTRxReadIndex = 0;
    ui_gui32UARTRxWriteIndex = 0;
...
uiUARTFlushTx(bool bDiscard)
{
    ...
    if(bDiscard)
    {
        ...
        //
        ui_gui32UARTTxReadIndex = 0;
        ui_gui32UARTTxWriteIndex = 0;
...
uiUARTxIntHandler(void)
{
    ...
    //
    ui32Ints = MAP_UARTIntStatus(ui_gui32Base, true);
    MAP_UARTIntClear(ui_gui32Base, ui32Ints);
    ...
    if(ui32Ints & UART_INT_TX)
    {
        ...
        if(uiTX_BUFFER_EMPTY)
        {
            MAP_UARTIntDisable(ui_gui32Base, UART_INT_TX);
        }
    }
    ...
    if(ui32Ints & (UART_INT_RX | UART_INT_RT))
    {
        ...
        //
        while(MAP_UARTCharsAvail(ui_gui32Base))
        {
            //
            // Read a character
            //
            ...
            if(!uiRX_BUFFER_FULL)
            {
                ...
                //
                ui_gpcUARTRxBuffer[ui_gui32UARTRxWriteIndex] =
