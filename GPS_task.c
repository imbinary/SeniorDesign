...
void DKFlinInit( GPSlat, GPSlon, GPShead) {
...
//the following for-loop completes the predict and update stages
	for(i=0;i<=7;i++) {
			P[i]=Po[i]+R[i]/3;//error prediction Pk = a*(Pk-1)*a + Q, where R/3 is used as process noise
//update stage
...
//	X[5] = Xo[5] + (X[3]-Xo[3])*Xo[2]/IMUcon;//Xacc in decimilliGs
		...
		char bsm[80];
		sprintf(bsm,"___>>>>> g4: %7.1f, g5: %7.6f, g6: %7.6f",
				Gk[4],Gk[5],Gk[6]);
